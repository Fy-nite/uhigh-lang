...
as of V2, Micro-Assembly now has dual conditional label support
...
The MicroASM include system has several advanced features to help you organize your code and find files.

### Basic Include

#include "filename"

Includes the contents of the specified file into the current file.
Example: `#include "stdio.print"` - Includes the stdio/print.masm file

### Selective Include

#include "label" from "filename"

Includes only the specified label's section from the target file.
Example: `#include "printf" from "stdio.print"` - Includes only the printf function from the stdio/print.masm file

### Include Search Paths

When resolving includes, MicroASM searches in the following locations in order:

1. Executable directory's "root" folder (for standard libraries)
2. Directory relative to the current source file
3. Executable directory itself
4. Current working directory
5. Current working directory's "root" folder

### "Did You Mean?" Suggestions

When an include file cannot be found (especially for standard library files), MicroASM will suggest similar file names that might be what you intended.

Example:

Could not find 'stdio/printx.masm', but found these similar files:

Did you mean: print.masm?
Did you mean: printf.masm?

set_include_path "path"

Sets a custom path to search for include files.
Example: `set_include_path "/usr/local/lib/masm"` - Adds this path to the include search paths

### Verify Include Path

verify_include_path "filename"

Verifies that a file exists in the include path without actually including it.
Example: `verify_include_path "stdio.print"` - Checks if this file exists in the include paths